...
$q3=$_POST['t'];
	...
	if($_POST['check_list']==NULL)
	{
		if($q1 !=0 )
		{
			echo "<script>alert('please select item');window.location.href='restaurant3menu1.php';</script>";		
		}
		if($q2 !=0 )
		{
			echo "<script>alert('please select item');window.location.href='restaurant3menu1.php';</script>";		
	...
	elseif(!empty($_POST['check_list']))// Loop to store and display values of individual checked checkbox.
	{
		...
		foreach($_POST['check_list'] as $selected )
		{
			...
			if($selected=='31')
			{
				if($q1==NULL)
				{
					echo "<script>alert('please enter quantity');window.location.href='restaurant3menu1.php';</script>";
				}
				else
				{
			//echo "in if";
			$sql4 = "SELECT price FROM items WHERE iid='31'";
			...
			$new6 = "SELECT iname FROM items WHERE iid='31'";
			$nres6 = mysql_query($new6);
			$nvalue6 = mysql_fetch_object($nres6);

			$new7 = "SELECT total FROM orders WHERE iid='31'";
			...
			elseif($selected=='32')
			{
				...
				{
			//echo "in if";
			$sql4 = "SELECT price FROM items WHERE iid='32'";
			$res4 = mysql_query($sql4);
			$value4 = mysql_fetch_object($res4);
			...
			$new8 = "SELECT iname FROM items WHERE iid='32'";
			$nres8 = mysql_query($new8);
			$nvalue8 = mysql_fetch_object($nres8);

			$new9 = "SELECT total FROM orders WHERE iid='32'";
			...
			elseif($selected=='33')
			{
				...
				{
			//echo "in if";
			$sql4 = "SELECT price FROM items WHERE iid='33'";
			$res4 = mysql_query($sql4);
			$value4 = mysql_fetch_object($res4);
			...
			$new10 = "SELECT iname FROM items WHERE iid='33'";
			$nres10 = mysql_query($new10);
			$nvalue10 = mysql_fetch_object($nres10);

			$new11 = "SELECT total FROM orders WHERE iid='33'";
...
$sql7 = "INSERT INTO hbill3(sno,billamt,bdate) values(NULL,'$sum','$date')";
