...
$q3=$_POST['t'];
	...
	if($_POST['check_list']==NULL)
	{
		if($q1 !=0 )
		{
			echo "<script>alert('please select item');window.location.href='restaurant1menu1.php';</script>";		
		}
		if($q2 !=0 )
		{
			echo "<script>alert('please select item');window.location.href='restaurant1menu1.php';</script>";		
	...
	elseif(!empty($_POST['check_list']))// Loop to store and display values of individual checked checkbox.
	{
		...
		foreach($_POST['check_list'] as $selected )
		{
			...
			{	
				if($q1==NULL)
				{
					echo "<script>alert('please enter quantity');window.location.href='restaurant1menu1.php';</script>";
				}
				else
				{
				echo $q1;
				...
				$sql3 = "INSERT INTO orders(rid, iid,price, quantity, total) values('$value2->rid', '$selected', 				'$value4->price', '$q1', '300')"; 
				$res2=mysql_query($sql3);
				$pri = $value4->price;
				$sql5 = "UPDATE orders SET total=quantity*price";
				$res5 = mysql_query($sql5);
			
				$new6 = "SELECT iname FROM items WHERE iid='11'";
				$nres6 = mysql_query($new6);
				$nvalue6 = mysql_fetch_object($nres6);

				$new7 = "SELECT total FROM orders WHERE iid='11'";
			...
			elseif($selected=='12')
			{
				...
				else
				{
					...
					$res4 = mysql_query($sql4);
					$value4 = mysql_fetch_object($res4);
					$sql3 = "INSERT INTO orders(rid, iid,price, quantity, total) values('$value2->rid', '$selected', 				'$value4->price', '$q2', '300')"; 
					$res2=mysql_query($sql3);

					$sql5 = "UPDATE orders SET total=quantity*price";
					$res5 = mysql_query($sql5);	

					$new8 = "SELECT iname FROM items WHERE iid='12'";
					$nres8 = mysql_query($new8);
					$nvalue8 = mysql_fetch_object($nres8);

					$new9 = "SELECT total FROM orders WHERE iid='12'";
			...
			elseif($selected=='13')
			{
				...
				else
				{
					...
					$res4 = mysql_query($sql4);
					$value4 = mysql_fetch_object($res4);
					$sql3 = "INSERT INTO orders(rid, iid, price, quantity, total) values('$value2->rid', '$selected', 				'$value4->price', '$q3', '300')"; 
					$res2=mysql_query($sql3);

					$sql5 = "UPDATE orders SET total=quantity*price";
					$res5 = mysql_query($sql5);	
	
					$new10 = "SELECT iname FROM items WHERE iid='13'";
					$nres10 = mysql_query($new10);
					$nvalue10 = mysql_fetch_object($nres10);

					$new11 = "SELECT total FROM orders WHERE iid='13'";
...
$f1 = "SELECT billno FROM bill WHERE cid=$temp AND rid=$value2->rid"; //AND rid=$value2->rid;
$f2 = mysql_query($f1);
$f3 = mysql_fetch_object($f2);
//echo $fetch10;

$f4 = "SELECT billno FROM bill WHERE cid=$temp AND rid=$value2->rid"; //AND rid=$value2->rid;
$f5 = mysql_query($f4);
$f6 = mysql_fetch_object($f5);
$f7 = $f6->billno;
...
$sql7 = "INSERT INTO hbill1(sno,billamt,bdate) values(NULL,'$sum','$date')";
$res7 = mysql_query($sql7);
$sql101 = "INSERT INTO hhbill1(sno,billamt,bdate,cid,billno) values(NULL,'$sum','$date','$temp','$f7')";
...
echo"<html><center><br/><br/><h3>Your bill number is: $f3->billno<br><br>Your bill amount is: ₹$sum</h3></center></html>";
...
echo "<script>window.open('/fpdf181/test.php','_blank')</script>"; 
}
